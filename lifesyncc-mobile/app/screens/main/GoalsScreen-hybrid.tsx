...
import { useAuth } from '../../contexts/AuthContext-hybrid';
import { 
  collection, 
  query, 
  where, 
...
import { db } from '../../../shared/config/firebase-hybrid';
...
export const GoalsScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log(`Setting up goals listener for user: ${user.id} on ${Platform.OS}`);
    ...
    const unsubscribe = onSnapshot(goalsQuery, (snapshot) => {
      console.log(`Goals snapshot received (${Platform.OS}), docs count:`, snapshot.docs.length);
      ...
      setLoading(false);
      setRefreshing(false);
      console.log(`Goals updated (${Platform.OS}):`, goalsData.length);
    }, (error) => {
  ...
  const addSampleGoal = async (category: Goal['category']) => {
    ...
    const sampleGoals = {
      ...
      social: {
        ...
        type: 'habit' as const,
        targetValue: 4, // 4 times per month
        currentValue: 1,
      },
    };
    ...
    try {
      console.log(`Adding goal to Firebase (${Platform.OS}):`, sampleGoal.title);
      ...
      await addDoc(collection(db, 'goals'), {
        ...
        platform: Platform.OS, // Track which platform goal was created from
      });

      Alert.alert('Success!', `${sampleGoal.title} added successfully!`);
      console.log(`Goal added successfully from ${Platform.OS}`);
    } catch (error) {
      console.error('Error adding goal:', error);
      Alert.alert('Error', 'Failed to add goal. Please check your internet connection.');
    }
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.platformBadge}>
          <Text style={styles.platformText}>
            🔥 Firebase {Platform.OS === 'web' ? 'Web' : 'Native'} - Cross-platform sync!
          </Text>
        </View>
      </View>
      ...
      {goals.length === 0 && (
        <View style={styles.welcomeCard}>
          ...
          <Text style={styles.firebaseNote}>
            🔥 Cross-platform Firebase sync - Your goals work everywhere!
          </Text>
        </View>
      )}
    </ScrollView>
